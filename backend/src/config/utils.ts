...
const keysChecker = (obj: { [key: string]: any }, keys: string[]) => {
...
const decypherText = (cipherText: string, iv: string, key: string): string => {
...
const cypherTextWithKey = (text: string, key: string, iv: string): string => {
	const keyTemp = Buffer.from(key, "hex");
	const ivTemp = Buffer.from(iv, "hex");
